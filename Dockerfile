...
RUN apt-get update && apt-get -y upgrade && apt-get -y install git sudo curl unattended-upgrades python && curl -sL https://deb.nodesource.com/setup_4.x | sudo -E bash - && apt-get -f -y install nodejs
